...
(* build: (int*int->int) * int -> Expr 
   ...
   This function recursively build each part of the expression 
   until depth becomes 0 and randomly call buildX or buildY.
...
(* build2: (int*int->int) * int -> Expr 
   ...
   the expression.
*)                          
let rec build2 (rand,depth) = if depth = 0 then
                              let x = rand(0,2) in 
                                              match x with
                                              | 0 -> buildX()
                                              | 1 -> buildY()
                                              ...
                                              | 6 -> buildPositive((build(rand,depth-1)),(build(rand,depth-1)),(build(rand,depth-1)))
